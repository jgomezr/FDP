 ...
 * tableName - The table to search from.
 * Filters = Any number of filters may be specified for the search. Filters specify a column and
 * a condition that it must match for a record to be included in the result. Filters are ANDed
 * together by default, but a disjunction can be used and can be set by disjunction = true
 *Sorts - Any number of sorts may be specified. Each sort consists of a column, a flag for
 * ascending or descending and a flag for whether or not to ignore the case
 * Fields - The fields that are returned for each row in the result set.
 * Additionally, fields can be specified using column operators: COUNT, COUNT DISTINCT, SUM, AVG,
 * MAX, MIN. Note that fields with column operators can not be mixed with fields that do not use
 * column operators.
 * Set distinct to true in order to filter out duplicate results.
 * Paging - The maximum number of results may be specified with maxResults.
 * (This can also be thought of as results per page.) The first result can be specified using
 * either firstResult
 */
...
public class Search implements Serializable {
    ...
    //Add a filter that uses the IN operator. This takes a variable number of parameters.
    ...
    //Add a filter that uses the NOT IN operator. This takes a variable number of parameters.
    // Any number of values can be specified.
    ...
    //Add a filter that uses the AND operator. This takes a variable number of parameters.
    // Any number of Filter can be specified.
    public Search addFilterAnd(Filter... filters) {
        addFilter(Filter.and(filters));
        return this;
    }

    //Add a filter that uses the OR operator. This takes a variable number of parameters.
    // Any number of <code>Filter</code>s can be specified.
    ...
    //Filters added to a search are "ANDed" together if this is false (default) and "ORed" if it is
    // set to true.
    ...
    //Add sort by column. Ascending if desc == false, descending if desc == true.
    ...
    //If this field is used with resultMode == RESULT_MAP, the column will also be used as
